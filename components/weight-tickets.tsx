...
import { Eye, Plus, Printer, Search, Trash2, ArrowLeft } from 'lucide-react';
...
function Paper({ticket}:{ticket:Ticket}){
 ...
 <h3>Datos de peso en libras</h3><div className="weight-table-scroll"><table><thead><tr>{['Código','Residuo','Bruto','Tara','Neto','Precio / lb','Total valorizado'].map(x=><th key={x}>{x}</th>)}</tr></thead><tbody>{ticket.lines.map((l,i)=><tr key={i}><td>{l.code}</td><td>{l.material}</td><td>{weight(l.bruto)}</td><td>{weight(l.tara)}</td><td><b>{weight(l.neto)}</b></td><td>{money(l.price)}</td><td>{money(l.total)}</td></tr>)}{!ticket.lines.length&&<tr><td colSpan={7} className="weight-empty">Selecciona un residuo para visualizar el pesaje.</td></tr>}</tbody></table></div>
 <div className="weight-grand"><span>Peso neto: {weight(ticket.lines.reduce((s,l)=>s+l.neto,0))}</span><strong>Total: {money(ticket.total)}</strong></div>
...
export function WeightTickets({notify}:{notify:(s:string,t?:'success'|'error')=>void}){
 ...
 const [busy,setBusy]=useState(false),[error,setError]=useState(''),[search,setSearch]=useState('');const lock=useRef(false);
 useEffect(()=>{let live=true;(async()=>{try{if(configured){const {data,error}=await createClient().from('weight_tickets').select('document').order('created_at',{ascending:false});if(error)throw error;if(live)setTickets((data??[]).map(x=>x.document as Ticket))}else{const saved=localStorage.getItem(key);if(saved&&live)setTickets(JSON.parse(saved))}}catch(e){const message=e instanceof Error?e.message:(typeof e==='object'&&e&&'message' in e?String((e as {message?:unknown}).message??''):'');if(live)setError(message||'No se pudieron cargar las boletas.')}finally{if(live)setReady(true)}})();return()=>{live=false}},[configured,user?.id]);
 ...
 const filteredTickets=normalizedSearch?tickets.filter(ticket=>[
  ticket.code,
  date(ticket.entered),
  ticket.kind==='supplier'?'Proveedor':'Cliente',
  ticket.party,
  ticket.party_code,
  ticket.operator,
  ...ticket.lines.flatMap(line=>[line.code,line.material,line.category??''])
 ].join(' ').toLocaleLowerCase('es').includes(normalizedSearch)):tickets;
 ...
 }catch(e){const message=e instanceof Error?e.message:(typeof e==='object'&&e&&'message' in e?String((e as {message?:unknown}).message??''):'');setError(message||'No fue posible emitir la boleta.')}finally{lock.current=false;setBusy(false)}};
 ...
 <Button disabled={busy||!lines.length} onClick={emit}>{busy?'Guardando…':'Emitir boleta y actualizar inventario'}</Button></section><aside className="weight-preview"><div className="weight-preview-title"><span className="weight-live"/>Vista previa en tiempo real</div><Paper ticket={preview}/></aside></div>:selected?<><div className="weight-toolbar"><Button variant="outline" onClick={()=>setSelected(null)}><ArrowLeft/>Historial</Button><Button onClick={()=>window.print()}><Printer/>Imprimir / PDF</Button></div><Paper ticket={selected}/></>:<section className="panel"><div className="weight-history-head"><div><h2>Boletas emitidas</h2><p>{tickets.length} {tickets.length===1?'registro':'registros'}</p></div><label className="weight-search"><Search aria-hidden="true"/><span className="sr-only">Buscar boletas</span><input type="search" value={search} onChange={event=>setSearch(event.target.value)} placeholder="Buscar boleta, nombre, tipo o material…" aria-label="Buscar en las boletas emitidas"/></label></div><div className="table-wrap"><table><thead><tr><th>Boleta</th><th>Ingreso</th><th>Tipo</th><th>Nombre</th><th>Neto</th><th>Total</th><th>Acciones</th></tr></thead><tbody>{filteredTickets.map(t=><tr key={t.id}><td>{t.code}</td><td>{date(t.entered)}</td><td>{t.kind==='supplier'?'Proveedor':'Cliente'}</td><td>{t.party}</td><td>{weight(t.lines.reduce((s,l)=>s+l.neto,0))}</td><td>{money(t.total)}</td><td><Button variant="ghost" onClick={()=>setSelected(t)}><Eye/>Ver</Button></td></tr>)}{!tickets.length&&<tr><td colSpan={7} className="weight-empty">{ready?'Crea tu primera boleta de peso.':'Cargando boletas…'}</td></tr>}{tickets.length>0&&!filteredTickets.length&&<tr><td colSpan={7} className="weight-empty">No se encontraron boletas con “{search.trim()}”.</td></tr>}</tbody></table></div></section>}</div>;
}
