...
import { defaultMaterialGroups, materialBelongsToGroup, type MaterialGroup } from '@/lib/material-groups';
...
export function MaterialCatalog({notify}:{notify:(message:string,tone?:'success'|'error')=>void}) {
  ...
  {[...groups,...(materials.some(m=>!groups.some(g=>materialBelongsToGroup(m,g)))?[{id:'',name:'Materiales sin clasificar',active:true}]:[])].map(g=><article className="material-group" key={g.id}>
    ...
    <ul>{materials.filter(m=>g.id?materialBelongsToGroup(m,g):!groups.some(group=>materialBelongsToGroup(m,group))).map(m=><li key={m.id}><span>{m.name}<small style={{display:'block'}}>{materialDisplayId(m)}</small></span><small>{m.active?'Activo':'Inactivo'}</small><label className="toggle"><input aria-label={'Activar '+m.name} type="checkbox" checked={m.active} onChange={()=>void toggle(m)}/><i/></label><div className="catalog-item-actions"><Button variant="ghost" size="icon" aria-label={'Editar material '+m.name} onClick={()=>open('material',g.id,m)}><Edit3/></Button><Button variant="ghost" size="icon" aria-label={'Eliminar material '+m.name} onClick={()=>{setDeleting({kind:'material',id:m.id,name:m.name});setDeleteError('')}}><Trash2/></Button></div></li>)}</ul>
    {!materials.some(m=>g.id?materialBelongsToGroup(m,g):!groups.some(group=>materialBelongsToGroup(m,group)))&&<p>No hay tipos de material registrados en esta categoría.</p>}
