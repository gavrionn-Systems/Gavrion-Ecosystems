...
export function Certificates(){
 const {settings,user,recordDocumentActivity}=useEconexoData();
 ...
 const printCertificate=async()=>{await recordDocumentActivity('certificate').catch(()=>undefined);window.print();};
 return <div className="certificate-module">
 <div className="page-title editorial-title"><div><p className="eyebrow">DOCUMENTOS DE LA EMPRESA</p><h1>Certificado</h1><p>Redacta tu documento y revisa la vista previa antes de imprimir.</p></div><Button disabled={!text.trim()} onClick={()=>void printCertificate()}><Printer/>Imprimir / PDF</Button></div>
